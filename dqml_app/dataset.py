...
class ModelParameters:
    ...
    def __init__(self, features: list[Feature]|list[dict], hist_data_snapshots: list[DataSnapshot]|list[dict], sample_size: int):
        if isinstance(features, list) and all(isinstance(feature, dict) for feature in features):
            self.features = [Feature(**feature) for feature in features]
        else:
            self.features = features

        if isinstance(hist_data_snapshots, list) and all(isinstance(snapshot, dict) for snapshot in hist_data_snapshots):
            self.hist_data_snapshots = [DataSnapshot(**snapshot) for snapshot in hist_data_snapshots]
        else:
...
class Dataset:
    ...
    def __init__(self, dataset_id: str, catalog_ind: bool, schedule_id: str, model_parameters: ModelParameters|dict):
        self.kind = "generic"
...
class DelimFileDataset(Dataset):
    ...
    def __init__(self, dataset_id: str, catalog_ind: bool, schedule_id: str, model_parameters: ModelParameters|dict, file_delim: str):
...
class LocalDelimFileDataset(DelimFileDataset):
    ...
    def __init__(
        self, dataset_id: str, catalog_ind: bool, schedule_id: str, model_parameters: ModelParameters|dict, file_delim: str, file_path: str
    ):
        super().__init__(dataset_id, catalog_ind, schedule_id, model_parameters, file_delim)
...
class AWSS3DelimFileDataset(DelimFileDataset):
    ...
    def __init__(
        self, dataset_id: str, catalog_ind: bool, schedule_id: str, model_parameters: ModelParameters|dict, file_delim: str, s3_uri: str
    ):
        super().__init__(dataset_id, catalog_ind, schedule_id, model_parameters, file_delim)
...
class AzureADLSDelimFileDataset(DelimFileDataset):
    ...
    def __init__(
        self, dataset_id: str, catalog_ind: bool, schedule_id: str, model_parameters: ModelParameters|dict, file_delim: str, adls_uri: str
    ):
        super().__init__(dataset_id, catalog_ind, schedule_id, model_parameters, file_delim)
